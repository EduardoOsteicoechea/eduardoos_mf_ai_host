...
todo_app_add_task_button.addEventListener("click", () => {
  if (todo_app_add_task_name_input.value === "") {
    alert("Add a value to the task")
    return
  }
  let new_task_text = todo_app_add_task_name_input.value.trim()
  const now = new Date()
  ...
  todo_app_add_task_name_input.focus()
})
...
function regenerate_todo_list() {
  ...
  todo_app_add_task_name_input.value = ""
}
...
function task_markup(new_task_index, current_task_data) {
  ...
  new_task_item_checkbox_container.classList.add("new_task_item_checkbox_container_unchecked")
